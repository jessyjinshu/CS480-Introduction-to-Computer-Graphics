...
//****************************************************************************
// 
//   December 11, 2018 Created by Yuanpei Wang
//
public class Vector3D
{
	...
	public float dist(Vector3D v){
		float res = (float)Math.sqrt(Math.pow(x-v.x, 2) + Math.pow(y-v.y, 2)+Math.pow(z-v.z, 2));
		return res;
	}
	public void rotateVector(Quaternion q, Vector3D center)
	{
		...
		Quaternion p;
				...
				p = new Quaternion((float)0.0,this.minus(center)); 
				p=q.multiply(p);
				p=p.multiply(q_inv);
				vec = p.get_v();
				vec=vec.plus(center);
