...
public class SketchBase 
{
	...
	public static void drawPoint(BufferedImage buff, Point3D p)
	{
		if(p.x>=0 && p.x<buff.getWidth() && p.y>=0 && p.y < buff.getHeight()){
			if(p.z > lab9.depthbuff.getDepth(p.x,p.y)){
			lab9.depthbuff.setDepth(p.x,p.y,p.z);
	 ...
	 */
	public static void drawLine(BufferedImage buff, Point3D p1, Point3D p2)
	{
	    ...
	    if(x_y_role_swapped)
	    {
	    	if(x>=0 && x<buff.getHeight() && y>=0 && y<buff.getWidth()){
	    		if(z > lab9.depthbuff.getDepth(y,x)){ 
	    			lab9.depthbuff.setDepth(y,x,z);
	    ...
	    else
	    {
	    	if(y>=0 && y<buff.getHeight() && x>=0 && x<buff.getWidth()){
	    		if(z > lab9.depthbuff.getDepth(x,y)){ 
	    		buff.setRGB(x, buff.getHeight()-y-1, (r<<16) | (g<<8) | b);
	    		lab9.depthbuff.setDepth(x,y,z);
	    		}
	    	}
	    }
	    ...
	    while (x != xEnd) 
	    {
	    	...
	    	{
	    		if(x>=0 && x<buff.getHeight() && y>=0 && y<buff.getWidth()){
	    			//System.out.printf("x: %d, y: %d, z: %d , depth buff: %d \n",x,y,z,lab9.depthbuff.getDepth(y,x));
	    			if (z > lab9.depthbuff.getDepth(y,x)){ 	
	    			buff.setRGB(y, buff.getHeight()-x-1, (r<<16) | (g<<8) | b);
	    			lab9.depthbuff.setDepth(y,x, z);
	    	}
	 ...
	 */
	public static void drawTriangle(BufferedImage buff, Point3D p1, Point3D p2, Point3D p3, boolean do_smooth)
	{
	 ...
	 */
	public static Point3D[] sortTriangleVerts(Point3D p1, Point3D p2, Point3D p3)
	{
